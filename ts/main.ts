...
$(() => {
  ...
  $("#btn-calc-tax").on("click", () => {
    ...
    if ($("#income-input .detail-box").css("display") != "none") {
      const input_bonus_income_once = Number(
        $("#input-bonus-income-once").val()
      );
      const input_basic_work_hours = Number($("#input-basic-work-hours").val());
      const input_overwork_rate = Number($("#input-overwork-rate").val());
      const input_extreme_overwork_rate = Number(
        $("#input-extreme-overwork-rate").val()
      );
     ...
     * --------------------------------------------------*/
    //給料の元になる支給額の計算（ボーナス）
    const bonus_income_total: number = Math.floor(
      bonus_income_once * bonus_number
    );
    //給料の元になる支給額の計算（時間外労働手当）
    const overwork_monthly_income: number = calcOverworkIncome(
      income,
      overwork_hours,
      basic_work_hours,
      overwork_rate,
    ...
    r.find("[overwork-monthly-income]").text(
      add1000Separator(overwork_monthly_income)
    );
     ...
     * --------------------------------------------------*/
    // 健康保険料
    let hi: HealthInsurance = new HealthInsurance(
      hi_rate,
      income,
      monthly_income
    );
    let hi_bonus: HealthInsurance = new HealthInsurance(
      hi_rate,
      income,
      bonus_income_total,
      bonus_number
    ...
    const hi_annually: DataSetForInsurancePremium = {
      ...
      company:
        hi.premium.company * 12 + hi_bonus.premium.company * bonus_number,
    ...
    let ep_bonus: EmployeePension = new EmployeePension(
      income,
      bonus_income_total,
      bonus_number
    );
    const ep_annually: DataSetForInsurancePremium = {
      ...
      company:
        ep.premium.company * 12 + ep_bonus.premium.company * bonus_number,
      total: ep.premium.total * 12 + ep_bonus.premium.total * bonus_number,
    };

    // 雇用保険料
    const ui: UnemplymentInsurance = new UnemplymentInsurance(
      industry_type,
      monthly_income
    );
    const ui_bonus: UnemplymentInsurance = new UnemplymentInsurance(
      industry_type,
      bonus_income_once
    );
    const ui_annually: DataSetForInsurancePremium = {
      ...
      company:
        ui.premium.company * 12 + ui_bonus.premium.company * bonus_number,
      total: ui.premium.total * 12 + ui_bonus.premium.total * bonus_number,
    };

    // 社会保険料の天引き月額
    const premium_monthly: DataSetForInsurancePremium = sum(
      hi.premium,
      ep.premium,
      ui.premium
    );
    ...
    const premium_bonus: DataSetForInsurancePremium = sum(
      hi_bonus.premium,
      ep_bonus.premium,
      ui_bonus.premium
    );
    ...
    const premium_annually: DataSetForInsurancePremium = {
      ...
      company:
        premium_monthly.company * 12 + premium_bonus.company * bonus_number,
      total: premium_monthly.total * 12 + premium_bonus.total * bonus_number,
    };

    // 結果を出力（社会保険料）
    r.find("[hi-half]").text(add1000Separator(hi.premium.you));
    r.find("[hi-half-bonus]").text(
      add1000Separator(hi_bonus.premium.you * bonus_number)
    );
    ...
    r.find("[ep-half-bonus]").text(
      add1000Separator(ep_bonus.premium.you * bonus_number)
    );
    ...
    r.find("[ui-you-bonus]").text(
      add1000Separator(ui_bonus.premium.you * bonus_number)
    );
    ...
    r.find("[taxiable-standard]").text(
      add1000Separator(it.taxable_standard_income)
    );
     ...
     * 住民税計算
     * --------------------------------------------------*/
    const rt: ResidentTax = new ResidentTax(
      annual_income,
     ...
     * --------------------------------------------------*/
    // 月収の社会保険料等控除後金額
    const salary_without_premium: number = Math.max(
      monthly_income - premium_monthly.you,
      0
    );

    // 月収の源泉徴収額（甲種）
    const salary_withholding: WithholdingSalary = new WithholdingSalary(
      salary_without_premium,
      false,
      0
    );

    // ボーナスの社会保険料等控除後金額
    const bonus_without_premium: number = Math.max(
      bonus_income_once - premium_bonus.you,
      0
    );

    // ボーナスの源泉徴収額
    const bonus_withholding: WithholdingBonus = new WithholdingBonus(
      bonus_without_premium,
      salary_without_premium,
      true,
      0
    );
    ...
    // 結果を出力
    r.find("[it-withholding]").text(
      add1000Separator(salary_withholding.tax_value)
    );
    r.find("[it-bonus-withholding]").text(
      add1000Separator(bonus_withholding.tax_value)
    );
     ...
     * --------------------------------------------------*/
    // 実質の年収
    const substantial_annual_income: number =
      annual_income - premium_annually.you - it.tax;
    // - residents_tax;は2年目以降

    // 実質のボーナス（1回あたり）
    const substantial_bonus: number =
      bonus_income_once - premium_bonus.you - bonus_withholding.tax_value;

    // 実質毎月振り込まれる月給
    const substantial_income: number =
      monthly_income - premium_monthly.you - salary_withholding.tax_value;

    // 結果を出力
    r.find("[substantial-income]").text(add1000Separator(substantial_income));
    r.find("[substantial-income-minus-rt]").text(
      add1000Separator(substantial_income - rt.tax_monthly)
    );
    r.find("[substantial-bonus]").text(add1000Separator(substantial_bonus));
    r.find("[substantial-annual-income]").text(
      add1000Separator(substantial_annual_income)
    );
    r.find("[substantial-annual-income-minus-rt]").text(
      add1000Separator(substantial_annual_income - rt.tax.total)
    );
    ...
    const message_body: string =
      "あなたの年収は「" +
      add1000Separator(annual_income) +
      "円」です。" +
      "\n" +
      "が、社会保険料と所得税が引かれて「" +
      add1000Separator(substantial_annual_income) +
      "円」ぐらいになります。" +
      "\n" +
      "去年と同じ年収なら、住民税も引いた手取り年収は「" +
      add1000Separator(substantial_annual_income - rt.tax.total) +
      "円」ぐらいです。";
    const site_url: string = "https://tax-calc.oguemon.com/";
    const hashtag: string = "ザックリ手取り給料計算機";
    const user_id: string = "oguemon_com";
    const textarea_body: string =
      message_body + "\n" + site_url + " #" + hashtag;
    const twitter_parms: string =
      "url=" +
      encodeURI(site_url) +
      "&text=" +
      encodeURI(message_body) +
      "&hashtags=" +
      encodeURI(hashtag) +
      "&related=" +
      user_id;
    ...
    const mail_body: string =
      "親戚各位" +
      "%0D%0A" +
      "%0D%0A" +
      "お世話になっております、〇〇です。" +
      "%0D%0A" +
      "ところでなのですが、ついに私の年収がわかりました。" +
      "%0D%0A" +
      "額面年収が「" +
      add1000Separator(annual_income) +
      "円」なのに対して、" +
      "所得税や社会保険料を引かれた後に残る金額は「" +
      add1000Separator(substantial_annual_income) +
      "円」くらいだそうです。" +
      "%0D%0A" +
      "さらに、この年収だと住民税が大体「" +
      add1000Separator(rt.tax.total) +
      "円」くらい課されるので、" +
      "昨年度と年収が変わらなかったら、手取り年収はおおよそ「" +
      add1000Separator(substantial_annual_income - rt.tax.total) +
      "円」になります。" +
      "%0D%0A" +
     ...
     * --------------------------------------------------*/
    // 過去にグラフ描画をしていたらそれを破棄
    if (chart_detail_income) {
      chart_detail_income.destroy();
    }
    if (chart_detail_bonus) {
      chart_detail_bonus.destroy();
    }
    if (chart_detail_annual_income) {
      chart_detail_annual_income.destroy();
    }
    if (chart_overwork_transition) {
      chart_overwork_transition.destroy();
    }
    ...
    const ctx_income_canvas: HTMLCanvasElement = <HTMLCanvasElement>(
      $("#graph-income-detail")[0]
    );
    const ctx_income: CanvasRenderingContext2D =
      ctx_income_canvas.getContext("2d");
    ...
    const ctx_bonus_canvas: HTMLCanvasElement = <HTMLCanvasElement>(
      $("#graph-bonus-income-detail")[0]
    );
    const ctx_bonus: CanvasRenderingContext2D =
      ctx_bonus_canvas.getContext("2d");
    ...
    const ctx_annual_canvas: HTMLCanvasElement = <HTMLCanvasElement>(
      $("#graph-annual-income-detail")[0]
    );
    const ctx_annual: CanvasRenderingContext2D =
      ctx_annual_canvas.getContext("2d");
    ctx_annual.canvas.height = 150;
    chart_detail_annual_income = plotDoughnutChart(
      ctx_annual,
      datasets_annual_income
    );
    ...
    for (let i = 0; i <= 100; i += 20) {
      ...
      // 額面年収
      const additional_income: number = calcOverworkIncome(
        income,
        i,
        basic_work_hours,
        overwork_rate,
      ...
      // 社会保険料
      const hi_over: HealthInsurance = new HealthInsurance(
        hi_rate,
        income,
        monthly_income
      );
      const ep_over: EmployeePension = new EmployeePension(
        income,
        monthly_income
      );
      const ui_over: UnemplymentInsurance = new UnemplymentInsurance(
        industry_type,
        monthly_income
      );

      const total_premium: number =
        (hi_over.premium.you + ep_over.premium.you + ui_over.premium.you) * 12 +
        premium_bonus.you * bonus_number;
    ...
    // グラフを描画（残業時間）
    const chart_overwork_transition_canvas: HTMLCanvasElement = <
      HTMLCanvasElement
    >$("#graph-overwork-income-transition")[0];
     ...
     * --------------------------------------------------*/
    // 課税所得金額 - 人的控除差調整額
    const income_tax_rate: Data.RateAndDeduction = Data.getIncomeTaxRate(
      rt.taxable_income - rt.diff_personal_deduction
    );
    // ふるさと納税の控除上限額を求める
    const frst_max_tax: number =
      2000 + (rt.income_tax.total * 0.2) / (0.9 - income_tax_rate.rate);
    // 結果を出力
  ...
  const calcOverworkIncome = (
    ...
    if (overwork_hours <= 60) {
      overwork_monthly_income =
        overwork_rate * overwork_hours * basic_income_per_hour;
    } else {
      overwork_monthly_income =
        (60 * overwork_rate + (overwork_hours - 60) * extreme_overwork_rate) *
        basic_income_per_hour;
    }
